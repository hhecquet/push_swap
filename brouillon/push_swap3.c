...
/*   Updated: 2024/12/18 16:56:15 by hhecquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	push_swap(int *tab_a, int *tab_b)
{
	...
	i = 0;
	tab_sort = tab_b;
	ft_sort_int_tab(tab_sort, g_size_b(0,1));
	...
	pb(tab_a, tab_b, g_size_a(0,1), g_size_b(0,1));
	if (tab_b[1] < tab_b[0] && check_order_plus(tab_a,tab_b) != 1)
		sb(tab_b);
	while (check_order_plus(tab_a,tab_b) != 1 && g_size_a(0,1) != 2)
	{
		var = tab_a[0];
		tab_sort = malloc(g_size_b(0,1) * sizeof(int));
		if (!tab_sort)
			return (0);
		i = 0;
		...
		ft_sort_int_tab(tab_sort, g_size_b(0,1));
		if (tab_a[0] < tab_b[chr_min(tab_b, g_size_b(0,1))] || tab_a[0] > tab_b[chr_max(tab_b, g_size_b(0,1))])
		{
			while (tab_b[0] != var && g_size_a(0,1) != 2)
			{
				if (chr_max(tab_b, g_size_b(0,1)) == 0)
					pb(tab_a, tab_b, g_size_a(0,1), g_size_b(0,1));
				else if (chr_max(tab_b, g_size_b(0,1)) < (g_size_b(0,1) - 1)/2)
					rb(tab_b, g_size_b(0,1));
				else if (chr_max(tab_b, g_size_b(0,1)) > (g_size_b(0,1) - 1)/2)
					rrb(tab_b, g_size_b(0,1));	
			}
		}
		else if ((tab_a[0] < tab_sort[(g_size_b(0,1))/2] && tab_b[0] >= tab_sort[(g_size_b(0,1) - 1)/2]) || (tab_a[0] > tab_sort[(g_size_b(0,1) - 1)/2] && tab_b[0] < tab_sort[(g_size_b(0,1) - 1)/2]))
		{
			while (tab_b[0] != var && g_size_a(0,1) != 2)
			{
	...
	if (chr_max(tab_a, g_size_a(0,1)) > chr_max(tab_b, g_size_b(0,1)))
	{
		if (g_size_a(0,1) == 2 && tab_a[0] < tab_a[1])
			sa(tab_a);
		else if (chr_max(tab_a, g_size_a(0,1)) > (g_size_a(0,1) - 1)/2)
		{
	...
	while (g_size_b(0,1) != 0)
	{
		...
		while (tab_a[g_size_a(0,1) - 1] < tab_a[0] && (tab_a[g_size_a(0,1) - 1] > tab_b[0] || g_size_b(0,1) == 0))
			rra(tab_a, g_size_a(0,1));
	}
	return (1);
}
